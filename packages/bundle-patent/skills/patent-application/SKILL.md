...
本技能只在用户明确要求推进申请文件（"写申请文件""准备提交""出权利要求书"这类点名表述）时启用。**"完成""写完""交付"这类未点名的指令默认指向交底书**——先走 patent-services 导出交底书 docx，再向用户确认是否继续申请文件。不要替用户把"完成交底书"理解成"完成专利申请"：申请文件是从交底书再加工的提交材料，跳过交底书交付等于没有交付。

推进前置（不满足先补齐，不满足就申请文件开工也是无源之水）：

- 交底书八章定稿，`exports/` 下已有交底书导出物（或用户明确表示不需要导出物）；
- 附图齐备且经 patent-figure-design 的 subagent 视觉验收通过。
...
- 导出时核对工具返回的"内嵌附图 N 张"与 08 章声明条数一致，零附图或缺图先修命名与渲染再重导（patent-services 纪律）。
